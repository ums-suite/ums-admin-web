...
/**
 * App-wide runtime configuration (ADMIN-1). One injection token rather than importing
...
export interface AppConfig {
  ...
  /** Poll interval (ms) for the shared async-job progress pattern, ADMIN-7. */
  readonly jobPollIntervalMs: number;
  /** Poll interval (ms) for mid-session permission re-validation, ADMIN-5. */
  readonly permissionRevalidateIntervalMs: number;
}

export const DEFAULT_APP_CONFIG: AppConfig = {
  apiBaseUrl: environment.apiBaseUrl,
  jobPollIntervalMs: 4_000,
  permissionRevalidateIntervalMs: 60_000,
};

/**
 * Root-provided with a real default (`environment.apiBaseUrl`) so any `providedIn: 'root'`
 * service that transitively depends on it resolves cleanly in a unit test that never calls
 * `provideCoreHttp()` -- `provide-core-http.ts`'s explicit provider still overrides this at real
 * app bootstrap.
 */
