...
#
# Builds ums-admin-web's CSR-only bundle. Two-stage build, mirroring ums-core's own Dockerfile
...
# `dist/admin-web/browser` static output into a slim nginx image, so the final image ships
